...
const { normalizeNationality } = require('./lib/normalizeNationality.cjs');
...
function usage() {
  console.log('Uso: node update_nacionalidades_db.cjs [--empresaId UUID] [--apply] [--limit N]');
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === '--dry-run-json') { dryRunFile = args[i+1]; i++; }
  ...
  if (!apply) {
    ...
    if (dryRunFile) {
      const fs = require('fs');
      try {
        fs.writeFileSync(dryRunFile, JSON.stringify(changes, null, 2), 'utf8');
        console.log(`Relatório de preview salvo em: ${dryRunFile}`);
      } catch (err) {
        console.warn('Falha ao salvar relatório de preview:', err.message || err);
      }
    }
