#!/usr/bin/env node
/**
 * 🎯 SCRIPT 03 - GERAÇÃO DE WORKFLOWS N8N A PARTIR DE TAREFAS (Node.js)
 * =========================================================================
 * 
 * Gera workflows N8N executáveis a partir das análises de automation_opportunities.
 ...
 * 
 * Uso:
 * node 03_generate_n8n_from_tasks.js --empresaId=UUID_DA_EMPRESA
 * node 03_generate_n8n_from_tasks.js --empresaId=UUID --minScore=70 (só workflows com score >= 70)
 * node 03_generate_n8n_from_tasks.js --empresaId=UUID --personaId=UUID (só 1 persona)
 * 
 * Requisitos:
 * - NEXT_PUBLIC_SUPABASE_URL e NEXT_PUBLIC_SUPABASE_ANON_KEY
 * - Tabelas: automation_opportunities, personas_workflows
 * - Script 02.5 já executado (análises salvas)
 * 
...
if (!empresaIdArg) {
  ...
  console.log('\nUso:');
  console.log('  node 03_generate_n8n_from_tasks.js --empresaId=UUID_DA_EMPRESA');
  console.log('  node 03_generate_n8n_from_tasks.js --empresaId=UUID --minScore=70');
  console.log('  node 03_generate_n8n_from_tasks.js --empresaId=UUID --personaId=UUID');
  process.exit(1);
}
  ...
  let query = supabase
    ...
    .select(`
      *,
      ...
      personas_tasks!inner(id, title, description, task_type, priority)
    `)
...
/**
 * Salvar workflow em arquivo JSON
 */
async function saveWorkflowToFile(workflow, empresa, persona, task) {
  await fs.mkdir(OUTPUT_DIR, { recursive: true });

  // Nome do arquivo: EMPRESACODIGO_PERSONANOME_TASKTITLE.json
  const personaSlug = persona.full_name.replace(/\s+/g, '_');
  const taskSlug = task.title.replace(/[^a-zA-Z0-9\s]/g, '').replace(/\s+/g, '_').substring(0, 50);
  const filename = `${empresa.codigo}_${personaSlug}_${taskSlug}.json`;
 ...
 */
async function saveWorkflowToDatabase(workflow, empresaId, personaId, opportunityId, taskId) {
  const record = {
...
async function main() {
  ...
  const opportunities = await fetchAutomationOpportunities(empresaIdArg, personaIdArg, minScoreArg);
  ...
  for (const opportunity of opportunities) {
    ...
    const task = opportunity.personas_tasks;
    ...
    // Salvar em arquivo
    try {
      const filepath = await saveWorkflowToFile(workflow, empresa, persona, task);
      report.workflows_saved_to_file++;
      console.log(`   💾 Salvo em: ${path.basename(filepath)}`);
    } catch (error) {
  ...
  console.log(`Salvos em arquivo: ${report.workflows_saved_to_file}`);
  console.log(`Salvos no banco: ${report.workflows_saved_to_db}`);
  console.log(`\n📁 Workflows disponíveis em: ${OUTPUT_DIR}`);
  console.log('='.repeat(80));
