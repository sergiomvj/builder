// SCRIPT DE BIOGRAFIAS CORRIGIDO - USA DADOS REAIS DO SUPABASE + LLM
...
console.log('🚀 SCRIPT BIOGRAFIA REAL - COM LLM INTEGRATION');
console.log('=============================================');
...
async function saveBiografiaToSupabase(persona, biografiaData) {
  try {
    ...
    // Preparar dados para inserção na tabela personas_biografias
    const biografiaRecord = {
      persona_id: persona.id,
      biografia_completa: biografiaData.biografia_completa,
      historia_profissional: biografiaData.historia_profissional,
      motivacoes: biografiaData.motivacoes,
      desafios: biografiaData.desafios,
      objetivos_pessoais: biografiaData.objetivos_pessoais,
      soft_skills: biografiaData.soft_skills,
      hard_skills: biografiaData.hard_skills,
      educacao: biografiaData.educacao,
      certificacoes: biografiaData.certificacoes,
      idiomas_fluencia: biografiaData.idiomas_fluencia,
      experiencia_internacional: biografiaData.experiencia_internacional,
      redes_sociais: biografiaData.redes_sociais,
      updated_at: new Date().toISOString()
    };
    
    // Inserir ou atualizar biografia no Supabase
    ...
    console.log(`✅ Biografia salva no Supabase para persona ID: ${persona.id}`);
    return true;
    
  } catch (error) {
    console.error('❌ Erro ao salvar biografia no Supabase:', error);
    return false;
  }
}
