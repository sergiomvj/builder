...
class RAGGenerator {
    ...
    /**
     * Busca dados reais da empresa no banco Supabase
     */
    async buscarDadosEmpresa(empresaCodigo) {
        try {
            // 1. Buscar empresa
            ...
            if (empresaError || !empresa) {
                throw new Error(`Empresa ${empresaCodigo} não encontrada`);
            }
            ...
            // 3. Buscar competências das personas
            ...
            if (competenciasError) {
                throw new Error(`Erro ao buscar competências: ${competenciasError.message}`);
            }
    ...
    /**
     * Estrutura dados de personas reais do Supabase para RAG
     */
    estruturarPersonasReaisRAG(empresa, personas, competencias) {
        try {
            const ragPersonas = [];

            for (let i = 0; i < personas.length; i++) {
                const persona = personas[i];
                const personaCompetencias = competencias.filter(comp => comp.persona_id === persona.id);

                const ragPersona = {
                    id: persona.id,
                    persona_key: `persona_${persona.id}`,
                    nome_completo: persona.full_name,
                    categoria: persona.role,
                    especialidade: persona.specialty,
                    experiencia_anos: persona.experiencia_anos || 5,
                    
                    // Biografia estruturada
                    biografia: {
                        resumo: persona.biografia_completa ? persona.biografia_completa.substring(0, 200) + '...' : `Profissional ${persona.role} especializado em ${persona.specialty}`,
                        trajetoria_completa: persona.biografia_completa,
                        educacao: persona.educacao || `Formação em ${persona.specialty}`,
                        certificacoes: persona.certificacoes || []
                    },

                    // Competências categorizadas
                    competencias: {
                        tecnicas: personaCompetencias
                            .filter(comp => comp.tipo === 'tecnica')
                            .map((comp, idx) => ({
                                id: `${persona.id}_t_${idx+1}`,
                                nome: comp.nome,
                                categoria: 'tecnica',
                                nivel: comp.nivel || this.inferirNivelCompetencia(comp.nome, persona.experiencia_anos),
                                contexto: comp.descricao || this.gerarContextoCompetencia(comp.nome, persona.specialty)
                            })),
                        comportamentais: personaCompetencias
                            .filter(comp => comp.tipo === 'comportamental')
                            .map((comp, idx) => ({
                                id: `${persona.id}_c_${idx+1}`,
                                nome: comp.nome,
                                categoria: 'comportamental',
                                nivel: comp.nivel || this.inferirNivelCompetencia(comp.nome, persona.experiencia_anos),
                                contexto: comp.descricao || this.gerarContextoCompetencia(comp.nome, persona.role)
                            }))
                    },

                    // Metadados para busca
                    search_metadata: {
                        tags: this.gerarTagsPersona(persona, personaCompetencias),
                        embedding_text: this.gerarTextoEmbedding(persona, personaCompetencias, persona.biografia_completa),
                        update_timestamp: new Date().toISOString()
                    }
                };
            ...
            console.error(`❌ Erro na estruturação de personas RAG reais: ${error.message}`);
            return [];
        }
    }
    ...
    gerarTagsPersona(persona, competencias) {
        const tags = [persona.role || persona.categoria, persona.specialty || persona.especialidade];

        // Verificar se competencias é um array (dados reais) ou objeto (dados template)
        if (Array.isArray(competencias)) {
            // Dados reais do Supabase
            const tecnicas = competencias.filter(c => c.tipo === 'tecnica').slice(0, 3);
            const comportamentais = competencias.filter(c => c.tipo === 'comportamental').slice(0, 2);
            tags.push(...tecnicas.map(c => c.nome));
            tags.push(...comportamentais.map(c => c.nome));
        } else if (competencias && competencias.tecnicas && competencias.comportamentais) {
            // Dados template
            tags.push(...competencias.tecnicas.slice(0, 3)); // Top 3 competências técnicas
            tags.push(...competencias.comportamentais.slice(0, 2)); // Top 2 comportamentais
        }

        return [...new Set(tags.filter(Boolean))]; // Remove duplicatas e valores vazios
    }
    ...
    gerarTextoEmbedding(persona, competencias, analise_biografia) {
        const nomePersona = persona.full_name || persona.nome;
        const categoriaPersona = persona.role || persona.categoria;
        const especialidadePersona = persona.specialty || persona.especialidade;
        const experienciaPersona = persona.experiencia_anos || persona.experiencia || 5;

        const textos = [
            `${nomePersona} trabalha como ${categoriaPersona}`,
            especialidadePersona ? `especialista em ${especialidadePersona}` : "",
            `com ${experienciaPersona} anos de experiência`
        ];

        // Verificar se competencias é um array (dados reais) ou objeto (dados template)
        if (Array.isArray(competencias)) {
            // Dados reais do Supabase
            const tecnicas = competencias.filter(c => c.tipo === 'tecnica').map(c => c.nome);
            const comportamentais = competencias.filter(c => c.tipo === 'comportamental').map(c => c.nome);
            
            if (tecnicas.length > 0) textos.push(`competências técnicas: ${tecnicas.join(', ')}`);
            if (comportamentais.length > 0) textos.push(`competências comportamentais: ${comportamentais.join(', ')}`);
        } else if (competencias && competencias.tecnicas && competencias.comportamentais) {
            // Dados template
            if (competencias.tecnicas) textos.push(`competências técnicas: ${competencias.tecnicas.join(', ')}`);
            if (competencias.comportamentais) textos.push(`competências comportamentais: ${competencias.comportamentais.join(', ')}`);
            if (competencias.ferramentas) textos.push(`utiliza ferramentas: ${competencias.ferramentas.join(', ')}`);
        }

        // Adicionar dados de biografia se disponíveis
        if (analise_biografia) {
            if (typeof analise_biografia === 'string') {
                textos.push(analise_biografia.substring(0, 200)); // Limitar tamanho
            } else if (analise_biografia.educacao) {
                textos.push(`educação: ${analise_biografia.educacao}`);
                if (analise_biografia.idiomas && Array.isArray(analise_biografia.idiomas)) {
                    textos.push(`idiomas: ${analise_biografia.idiomas.join(', ')}`);
                }
            }
        }
    ...
    /**
     * Cria catálogo unificado de competências baseado nos dados reais do Supabase
     */
    criarCatalogoCompetenciasReais(competenciasData) {
        const catalogoCompetencias = [];
        const competenciasUnicas = new Map();
        let compId = 1;
        ...
        competenciasData.forEach(comp => {
            ...
            if (!competenciasUnicas.has(chave)) {
                competenciasUnicas.set(chave, {
                    ...
                    nome: comp.nome,
                    categoria: comp.categoria,
                    contexto: comp.descricao || `Competência ${comp.categoria}: ${comp.nome}`,
                    personas_possuidoras: [],
                    nivel_medio: comp.nivel || 'intermediário'
                });
            }

            // Adicionar persona à competência
            const competencia = competenciasUnicas.get(chave);
            competencia.personas_possuidoras.push(comp.persona_id);
        });

        return Array.from(competenciasUnicas.values());
    }

    /**
     * Cria catálogo de ferramentas baseado em dados reais
     */
    criarCatalogoFerramentasReais(ragPersonas, techSpecsData) {
        const catalogoFerramentas = [];
        const ferramentasUnicas = new Map();
        let toolId = 1;

        // Extrair ferramentas das competências técnicas dos personas
        ragPersonas.forEach(persona => {
            if (persona.competencias && persona.competencias.tecnicas) {
                persona.competencias.tecnicas.forEach(comp => {
                    // Inferir ferramentas da competência técnica
                    const ferramentasInferidas = this.inferirFerramentasDeCompetencia(comp.nome);
                    
                    ferramentasInferidas.forEach(ferramenta => {
                        if (!ferramentasUnicas.has(ferramenta)) {
                            const specs = this.buscarSpecificacaoFerramenta(ferramenta, techSpecsData);
                            
                            ferramentasUnicas.set(ferramenta, {
                                id: toolId++,
                                nome: ferramenta,
                                categoria: this.categorizarFerramenta(ferramenta),
                                especificacao_tecnica: specs,
                                personas_utilizadoras: [],
                                frequencia_uso: 'média'
                            });
                        }

                        const tool = ferramentasUnicas.get(ferramenta);
                        tool.personas_utilizadoras.push(persona.nome_completo);
                    });
                });
            }
        });
    ...
    /**
     * Infere ferramentas baseado no nome da competência
     */
    inferirFerramentasDeCompetencia(competencia) {
        const inferencias = {
            'Python': ['Python', 'PyCharm', 'Jupyter', 'Git'],
            'JavaScript': ['JavaScript', 'Node.js', 'VS Code', 'Git'],
            'Marketing Digital': ['Google Analytics', 'HubSpot', 'Facebook Ads', 'LinkedIn'],
            'Vendas': ['Salesforce', 'HubSpot', 'LinkedIn Sales Navigator', 'Pipedrive'],
            'Excel': ['Microsoft Excel', 'Google Sheets'],
            'SQL': ['SQL Server', 'MySQL', 'PostgreSQL'],
            'Liderança': ['Slack', 'Microsoft Teams', 'Asana', 'Trello'],
            'default': ['Microsoft Office', 'Email', 'Web Browser']
        };

        for (const [key, ferramentas] of Object.entries(inferencias)) {
            if (competencia.toLowerCase().includes(key.toLowerCase())) {
                return ferramentas;
            }
        }

        return inferencias.default;
    }

    /**
     * Categoriza uma ferramenta
     */
    categorizarFerramenta(ferramenta) {
        const categorias = {
            'Python': 'linguagem_programacao',
            'JavaScript': 'linguagem_programacao',
            'Node.js': 'framework',
            'Salesforce': 'crm',
            'HubSpot': 'marketing_automation',
            'Google Analytics': 'analytics',
            'Microsoft Excel': 'produtividade',
            'Slack': 'comunicacao',
            'default': 'ferramenta_geral'
        };

        return categorias[ferramenta] || categorias.default;
    }
    ...
    gerarEstruturaPesquisa(ragPersonas, catalogoCompetencias, catalogoFerramentas) {
        ...
        ragPersonas.forEach(persona => {
            ...
            if (persona.ferramentas && persona.ferramentas.length > 0) {
                persona.ferramentas.forEach(ferramenta => {
    ...
    gerarContextosBuscaRAG(ragPersonas, catalogoCompetencias, catalogoFerramentas) {
        ...
        catalogoCompetencias.forEach(comp => {
            const personasComCompetencia = comp.personas_possuidoras && comp.personas_possuidoras.length > 0 
                ? comp.personas_possuidoras.map(p => typeof p === 'object' ? p.nome : p).join(', ')
                : 'Nenhuma persona';
            const ferramentasRelacionadas = comp.ferramentas_relacionadas && comp.ferramentas_relacionadas.length > 0
            ...
            contextos.push({
                ...
                conteudo: `${comp.contexto}. Personas com esta competência: ${personasComCompetencia}. Ferramentas relacionadas: ${ferramentasRelacionadas}.`,
                metadata: {
                    categoria: comp.categoria,
                    nivel_medio: comp.nivel_medio || 'intermediário',
                    total_personas: comp.personas_possuidoras ? comp.personas_possuidoras.length : 0
                },
        ...
        catalogoFerramentas.forEach(tool => {
            const usuariosFerrament = tool.usuarios && tool.usuarios.length > 0
                ? tool.usuarios.map(u => typeof u === 'object' ? u.nome : u).join(', ')
                : 'Nenhum usuário';
            const casosUso = tool.casos_uso && tool.casos_uso.length > 0
                ? tool.casos_uso.join(', ')
                : 'Nenhum caso de uso definido';
                
            contextos.push({
                id: `ferramenta_${tool.id}`,
                tipo: "ferramenta_usage",
                titulo: `Uso da ferramenta ${tool.nome}`,
                conteudo: `${tool.nome} é utilizada para ${tool.categoria_uso || tool.categoria || 'uso geral'}. Casos de uso identificados: ${casosUso}. Usuários: ${usuariosFerrament}.`,
                metadata: {
                    categoria_uso: tool.categoria_uso || tool.categoria,
                    total_usuarios: tool.usuarios ? tool.usuarios.length : 0,
                    casos_uso_count: tool.casos_uso ? tool.casos_uso.length : 0
                },
                tags: [tool.nome, tool.categoria_uso || tool.categoria, ...(tool.casos_uso || [])]
            });
        });
    ...
    async processarRAGDatabase(empresaCodigo, inputDir = null, outputPath = null) {
        try {
            ...
            // Buscar dados reais da empresa no Supabase
            const { empresa, personas, competencias } = await this.buscarDadosEmpresa(empresaCodigo);

            console.log(`📊 Processando ${personas.length} personas para RAG`);

            // 1. Estruturar personas para RAG usando dados reais
            const ragPersonas = this.estruturarPersonasReaisRAG(empresa, personas, competencias);
            console.log(`✅ Estruturadas ${ragPersonas.length} personas RAG`);

            // Gerar nomes reais para output
            ragPersonas.forEach(persona => {
                console.log(`✅ RAG gerado: ${persona.nome_completo} (${persona.categoria})`);
            });

            // 2. Criar catálogo de competências
            const catalogoCompetencias = this.criarCatalogoCompetenciasReais(competencias);
            console.log(`✅ Criado catálogo com ${catalogoCompetencias.length} competências únicas`);

            // 3. Criar catálogo de ferramentas (buscar do arquivo tech specs se disponível)
            const techSpecsPath = path.join(baseDir, 'tech_specs_output', `tech_specs_${empresaCodigo}.json`);
            let techSpecsData = null;
            try {
                techSpecsData = JSON.parse(await fs.readFile(techSpecsPath, 'utf8'));
            } catch (error) {
                console.log(`⚠️ Tech specs não encontrados: ${techSpecsPath}`);
            }

            const catalogoFerramentas = this.criarCatalogoFerramentasReais(ragPersonas, techSpecsData);
            ...
            let categorias = [];
            try {
                if ((await fs.stat(personasDir)).isDirectory()) {
                    categorias = await fs.readdir(personasDir);
                }
            } catch (error) {
                console.log(`⚠️ Diretório de personas não encontrado: ${personasDir}`);
                console.log(`📋 Processando apenas dados RAG consolidados...`);
            }
