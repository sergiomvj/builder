...
import { marked } from 'marked' // Para parsing de markdown
...
async function main() {
    ...
    if (!empresaId && !empresaCodigo) {
        console.error('❌ Erro: --empresaId ou --empresa-codigo é obrigatório');
        console.log('Uso: node 01_generate_competencias.js --empresaId EMPRESA_ID');
        process.exit(1);
    }

    try {
        const generator = new CompetenciasGenerator();
        // Usar empresaId se disponível, senão usar empresaCodigo
        const paramEmpresa = empresaId || empresaCodigo;
        const result = await generator.processarPersonas(paramEmpresa, outputPath);
