// Normalizes free-form nationality strings into a canonical single-value nationality.
// If nationalityText is ambiguous (e.g. 'Internacional'), we try to map from countryCode.
// Exports: normalizeNationality(nationalityText, countryCode) -> string
...
function normalizeNationality(nationalityText, countryCode) {
  ...
  const s = raw
    .toLowerCase()
    // normalize punctuation/hyphens and remove extra spaces
    .replace(/[-–—_\/]/g, '')
    .replace(/\s+/g, '');
  ...
  // If nothing matched, return a cleaned, capitalized version of the original
  // Try to keep multiple-word nationalities but normalize spacing
...
module.exports = { normalizeNationality };
