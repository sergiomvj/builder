...
export async function POST(request: NextRequest) {
  try {
    ...
    globalExecutionState = {
      isRunning: true,
      phase: 'Iniciando geração de fluxos de trabalho...',
      progress: '0/15',
      persona: '',
      error: null
    }

    const scriptPath = path.join(process.cwd(), 'AUTOMACAO', '02_PROCESSAMENTO_PERSONAS', 'generate_fluxos_sdr_simple.js')
    ...
    setTimeout(() => {
      if (globalExecutionState.isRunning) {
        globalExecutionState.phase = 'Analisando tarefas definidas...'
        console.log('🔄 Status atualizado:', globalExecutionState)
      }
    }, 2000)
    ...
    setTimeout(() => {
      if (globalExecutionState.isRunning) {
        globalExecutionState.phase = 'Criando fluxos N8N...'
        console.log('🔄 Status atualizado:', globalExecutionState)
      }
    }, 4000)
