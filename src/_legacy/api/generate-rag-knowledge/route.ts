...
export async function POST(request: NextRequest) {
  try {
    ...
    globalExecutionState = {
      isRunning: true,
      phase: 'Iniciando geração de base RAG...',
      progress: '0/15',
      persona: '',
      error: null
    }
    ...
    setTimeout(() => {
      if (globalExecutionState.isRunning) {
        globalExecutionState.phase = 'Analisando competências...'
    ...
    setTimeout(() => {
      if (globalExecutionState.isRunning) {
        globalExecutionState.phase = 'Criando base de conhecimento...'
    ...
    setTimeout(() => {
      if (globalExecutionState.isRunning) {
        globalExecutionState.phase = 'Estruturando RAG por persona...'
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Script de base RAG iniciado com sucesso',
