...
export async function POST(request: NextRequest) {
  try {
    ...
    globalExecutionState = {
      isRunning: true,
      phase: 'Iniciando script...',
      progress: '0/15',
      persona: '',
      error: null
    }

    const scriptPath = path.join(process.cwd(), 'AUTOMACAO', '02_PROCESSAMENTO_PERSONAS', '02_generate_competencias_supabase.js')
    ...
    setTimeout(() => {
      if (globalExecutionState.isRunning) {
        globalExecutionState.phase = 'Preparando ambiente...'
        console.log('🔄 Status atualizado:', globalExecutionState)
      }
    }, 2000)
    ...
    setTimeout(() => {
      if (globalExecutionState.isRunning) {
        globalExecutionState.phase = 'Executando script Node.js...'
        console.log('🔄 Status atualizado:', globalExecutionState)
      }
    }, 4000)
