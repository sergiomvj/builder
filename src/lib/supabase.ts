...
export function getSupabase() {
  ...
  const shouldUseServiceKey = supabaseServiceKey && supabaseServiceKey.startsWith('eyJ');
  const keyToUse = shouldUseServiceKey ? supabaseServiceKey : supabaseAnonKey;
  
  if (process.env.NODE_ENV === 'development') {
    if (!shouldUseServiceKey) {
       console.warn('WARN: Using Anon Key. Service Role Key missing or invalid. RLS may block writes.');
    } else {
